...
\section*{Introduction}
	...
	\begin{proof}
		...
		Let $A_x=\{i:X_i=x\mid X_0=x\}$.  For every $i\in A$, there is a $\P(x\to y)$ probability
		of transitioning from $x$ to $y$.  Thus,
		\[
			\P_x(N(y)=0) \leq \P(X_{i+1}\neq y\text{ for every }i\in A_x)=
			\prod_{i\in A_x} (1-\P(x\to y))=0
		\]
		since $|A_x|=\infty$.  We conclude that $\P_x(N(y)\geq 1)=1$ and so $\rho_{xy}=1$.
